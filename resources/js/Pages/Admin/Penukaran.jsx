  ...
  const filteredSortedPenukarans = useMemo(() => {
    ...
    if (search.trim() !== '') {
      ...
      filtered = filtered.filter(p => {
        ...
        const matchId = p.id.toString() === search.trim();
                ...
                <div className="flex justify-between items-center text-xs text-gray-600 space-x-2">
                  <div className="flex flex-col space-y-2">
                    <Link
                      ...
                      className="flex items-center max-w-[100%] truncate text-blue-600 hover:underline"
                    ...
                    {p.user?.pengelola_air?.nama && (
                      <Link
                        href={route('admin.pengelola-air.edit', p.user.pengelola_air.id)}
                        className="flex items-center space-x-1 text-blue-500 text-[11px] italic hover:underline"
                        title={`Edit data pengelola: ${p.user.pengelola_air.nama}`}
                      >
                        ...
                        <span>{p.user.pengelola_air.nama}</span>
                      </Link>
                    )}
                  </div>
