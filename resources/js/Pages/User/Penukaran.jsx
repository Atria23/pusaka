    ...
    const filteredSortedPenukarans = useMemo(() => {
        ...
        if (search.trim() !== '') {
            filtered = filtered.filter((p) =>
                p.voucher?.nama?.toLowerCase().includes(search.toLowerCase())
            );
        }
                        ...
                        return (
                            <div key={p.id} className="rounded-lg overflow-hidden shadow-md bg-white">
                                <div className="relative">
                                    ...
                                    <div
                                        className={`absolute top-3 right-3 text-xs font-semibold py-1 px-3 rounded ${p.status === 'sudah diredeem'
                                ...
                                <div className="p-4 pb-2 cursor-pointer" onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}>
                                    <h5 className={`text-main text-lg font-bold mb-1 ${expandedId === p.id ? 'break-words' : 'truncate'}`}>
                                        {v?.nama}
                                    </h5>
                                    <p className={`text-gray-700 text-sm mb-2 ${expandedId === p.id ? 'break-words line-clamp-none' : 'line-clamp-2'}`}>
                                        {v?.deskripsi}
                                    </p>
                ...
                <div className="flex justify-center items-center space-x-3 mb-8">
                    ...
                    {[...Array(totalPages)].map((_, i) => (
                        <button
                            ...
                            className={`px-3 py-1 rounded border ${currentPage === i + 1 ? 'bg-main text-white border-main' : 'border-gray-300'}`}
                        >
                            {i + 1}
                        </button>
                    ))}
