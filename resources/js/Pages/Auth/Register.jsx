...
export default function Register({ pengelolaAirList }) {
    const { data, setData, post, processing, errors, reset } = useForm({
        name: '',
        email: '',
        password: '',
                    ...
                    <section className="flex flex-col items-center justify-center gap-4 w-full px-4">
                        <form onSubmit={submit} className="w-full">
                            <div className="w-full h-max flex flex-col space-y-4 mb-6">
                                ...
                                <div>
    <label className="block text-gray-700 mb-1">Pilih Pengelola Air</label>
    <div className="w-full h-9 flex items-center rounded-lg bg-neutral-100 border-2 border-gray-200">
        <select
            id="pengelola_air_id"
            name="pengelola_air_id"
            value={data.pengelola_air_id}
            onChange={(e) => setData('pengelola_air_id', e.target.value)}
            className="bg-transparent text-sm border-none w-full h-full focus:ring-0 focus:outline-none placeholder-gray-400"
            required
        >
            <option value="" disabled>Pilih salah satu</option>
            {pengelolaAirList.map((item) => (
                <option key={item.id} value={item.id}>{item.nama}</option>
            ))}
        </select>
    </div>
    {errors.pengelola_air_id && <p className="text-red-500 text-sm mt-1">{errors.pengelola_air_id}</p>}
</div>
