...
//                     <p className="text-base text-gray-600 font-utama text-center">
...
export default function ResetPasswordOtp({ kontak }) {
    const sendOtpFirebase = async () => {
        const phone = data.kontak.replace(/^0/, '+62'); // ubah 08... jadi +628...
        ...
        try {
            ...
            alert('Gagal kirim OTP. Pastikan nomor valid dan koneksi baik.');
        }
    };
    ...
    const submit = async (e) => {
        ...
        try {
            // Verifikasi OTP pakai Firebase
            await window.confirmationResult.confirm(data.otp);

            // Jika verifikasi OTP sukses, lanjutkan ke backend Laravel
            post(route('password.otp.reset.submit'));
        } catch (error) {
            ...
            <div className="mx-auto w-full max-w-[500px] min-h-screen md:h-screen bg-white px-4 py-6">
                ...
                <section className="flex flex-col items-center justify-center gap-4 w-full px-4">
                    <form onSubmit={submit} className="w-full space-y-4 mb-6">
                        ...
                        <button
                            type="button"
                            onClick={sendOtpFirebase}
                            className="w-full bg-gray-100 border text-sm py-2 rounded text-main font-semibold"
                        >
                            Kirim OTP via Firebase
                        </button>
